...
export function ResetPasswordForm({ token }: ResetPasswordFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const repeatPasswordError = Validation.validatePasswordMatch(
      newPassword,
      repeatNewPassword,
    );
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid gap-3 relative">
            ...
            {repeatNewPassword && (
              <div
                className=" dark:text-white absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-zinc-900 transition-opacity duration-200"
                onClick={() => setIsViewRepeatPassword((prev) => !prev)}
              >
                {isViewRepeatPassword ? (
                  <PiHandEye size={20} />
                ) : (
                  <FaRegEyeSlash size={20} />
                )}
