...
async function verifyPasswordOrThrow(inputPassword: string, hashedPassword: string): Promise<void> {
...
function createResponse(token: string, email: string) {
    const res = NextResponse.json({message: `Welcome to ${email}`}, {status: 200});
